...
class TestUtils(unittest.TestCase):
    ...
    correct_V_T = np.array([0.0, 0.12471833, 0.25199507, 0.38189235, 0.5])
    ...
    def test_position(self):
        np.testing.assert_allclose(
            ...
            TestUtils.correct_position,
        )

    def test_velocity(self):
        np.testing.assert_allclose(
            geo_equatorial_frame_velocity(
                TestUtils.omega,
                TestUtils.i,
                TestUtils.w,
                TestUtils.h,
            ...
            TestUtils.correct_velocity,
        )

    def test_orb_elements(self):
        self.assertEqual(
            orb_elements(TestUtils.r, TestUtils.v, TestUtils.mu),
            TestUtils.correct_orb_elements,
        )

    def test_perigee_rad(self):
        self.assertEqual(
            perigee_rad(TestUtils.h_mag, TestUtils.mu, TestUtils.e_mag),
            TestUtils.correct_perigee,
        )

    def test_apogee_rad(self):
        self.assertEqual(
            apogee_rad(TestUtils.h_mag, TestUtils.mu, TestUtils.e_mag),
            TestUtils.correct_apogee,
        )

    def test_angular_momentum(self):
        self.assertEqual(
            angular_momentum(TestUtils.mu, TestUtils.ria, TestUtils.rip),
            TestUtils.correct_angular_momentum,
        )

    def test_period(self):
        self.assertEqual(
            period(TestUtils.mu, TestUtils.semimajor), TestUtils.correct_period
        )

    def test_gibbs(self):
        self.assertEqual(
            gibbs(TestUtils.r1, TestUtils.r2, TestUtils.r3, TestUtils.mu),
            TestUtils.correct_gibbs,
        )
